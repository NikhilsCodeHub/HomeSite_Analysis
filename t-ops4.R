...
write.csv(x = dtrain_final, file = "cleanTrainData.csv", row.names = FALSE)


write.csv(x = dtest_final, file = "cleanTestData.csv", row.names = FALSE)


##------
## -----Create Data Folds for Validation
##------

set.seed(2016)
folds_train <- createFolds(y=dtrain_final, k=5, list = TRUE, returnTrain = FALSE)

df <- lapply(X = folds_train, createDataframeFromFolds, dtrain_final)

fits <- lapply(df, createRPartFits)
...
#   saveRDS(lstFitsTrain[[1]],"fit_dfpersonal2.rds")
#   saveRDS(lstFitsTrain[[2]],"fit_dfsales2.rds")
#   saveRDS(lstFitsTrain[[3]],"fit_dfgeo2.rds")
#   saveRDS(lstFitsTrain[[4]],"fit_dfproperty2.rds")

##
## -- Generate Predictions on the iValidate Dataset
##

preds <- generatePredictions(fits, dtrain_final, "prob" )
...
dfCombo <- preds %>% tbl_df() %>% select(ends_with(".1"))

dfCombo <- cbind(dtrain_final$QuoteConversion_Flag, dfCombo)
...
# cfMx <- calcConfusionMx(dfCombo[,2:5], dfCombo$QuoteConversion_Flag)

##
## -- Generate Combo Fit
##

fitCombo <- createModelFits(dfCombo, "glm")
